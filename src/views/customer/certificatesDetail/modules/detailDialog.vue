<template>
  <div class="certificates-detail-detail-dialog">
    ...
    >
      <div class="m-conetnt">
        <el-row>
          <el-col :span="10">技术对接费：{{ruleForm.amtService}}元</el-col>
          <el-col :span="14">合同编号：{{ruleForm.contractNo}}</el-col>
        </el-row>

        <template v-if="this.modelType == 1">
          <!--BEGIN: 仲券详情modal -->
            ...
            >
              <el-table :data="tableData" border>
                ...
                <template v-else-if="tabsVal === '2'">
                  <el-table-column prop="ticketCountGift" label="获赠仲券" :key="11"></el-table-column>
                  <el-table-column prop="applyName" label="申请人" :key="12"></el-table-column>
                  <el-table-column prop="applyRemark" label="申请备注" :key="13">
                ...
                <el-table-column prop="auditRemark" label="审核结果">
                  <template slot-scope="scope">{{scope.row.auditRemark || '--'}}</template>
                </el-table-column>
              </el-table>
              <!-- 分页 -->
              <el-pagination
        ...
        <template v-if="this.modelType == 2">
          <!--BEGIN: 比例详情modal -->
          <el-tabs @tab-click="handleTabChange" v-model="tabsVal" type="border-card" class="mt-10">
            <el-tab-pane
              :label="item.label"
            ...
            >
              <el-table :data="tableData" border>
                <!-- 充值 -->
                <template v-if="tabsVal === '0'">
                  <el-table-column prop="ticketCount" label="充值服务费" :key="10"></el-table-column>
                </template>
                <!-- 赠券 -->
                <template v-else-if="tabsVal === '2'">
                  <el-table-column prop="ticketCountGift" label="获赠服务费" :key="11"></el-table-column>
                  <el-table-column prop="applyName" label="申请人" :key="12"></el-table-column>
                  <el-table-column prop="applyRemark" label="申请备注" :key="13">
                    <template slot-scope="scope">{{scope.row.applyRemark || '--'}}</template>
                  </el-table-column>
...
export default {
  ...
  methods: {
    ...
    // 开启
    open() {
      console.log("dialog-this.modelType::", this.modelType);
    },
    // 关闭浮层
    handleClose() {
