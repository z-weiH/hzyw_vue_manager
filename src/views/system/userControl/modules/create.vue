<template>
  <el-dialog
    ...
    <edits ref="edits1" :formname="'editsform1'" :edit-items="createItems"  :item="createItem" :label-width="'90px'">
        <el-form-item slot="addEdits" label="所属角色"  prop="roleids" :rules = "[{ required : true , message : '不能为空' , trigger : 'change'}]">
          <el-select v-model="createItem.roleids" multiple placeholder="请选择" >
            <el-option
    ...
    methods: {
      save() {
        this.checkbeforeSave().then(res=> {
            console.log(res);
            this.createItem.roleIds = this.createItem.roleids.join(',');
