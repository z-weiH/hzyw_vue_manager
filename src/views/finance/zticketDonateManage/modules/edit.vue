<template>
  <el-dialog :visible.sync="show" v-dialogDrag :title="title" width="890px" center>
        ...
        <table slot="tablePlus" class="m-primordial-table el-table el-table--fit el-table--border el-table--enable-row-hover mb-20">
          <tbody>
            <tr class="table-edits">
              <td colspan="4">
                <el-input v-if="editState === 1" type="textarea" v-model="item.apprerResult" placeholder="请输入审批原因"></el-input>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <span slot="footer" class="dialog-footer">
                ...
                <el-button v-if="$parent.editState == 1"  @click="saveAndcommit(0)">不通过</el-button>
...
export default {
    ...
    return {
      ...
      edtDefines: [
        ...
        },
        {
          ...
          title: '主管审批结果',
          content: [
            {label: '审批状态',type: 'text', columns: 1, property: 'managerStatus'},
            {label: '审批时间',type: 'date', columns: 1, property: 'managerTime'},
            {label: '审批原因',type: 'textarea', columns: 2, property: 'managerReason'},
          ]
        },{
        title: 'ceo审批结果',
          hidden: () => this.editState === 1,
          content: [
            {label: '审批状态',type: 'text', columns: 1, property: 'ceoStatus'},
            {label: '审批时间',type: 'date', columns: 1, property: 'ceoTime'},
            {label: '审批原因',type: 'textarea', columns: 2, property: 'ceoReason'},
          ]
        }
      ]
    };
  ...
  methods: {
    saveAndcommit(type) {
      if(!this.item.apprerResult && this.item.apprerResult !== 0){
        ...
        this.$http.post(URL_JSON['saveZticketDonateManage'],{apprerResult: this.item.apprerResult,isCommit: type,resultId: this.item.resultId})
          .then(res => {
            if(res.code === '0000'){
  ...
  created() {
    this.$http.post(URL_JSON['queryZticketCompany'])
      .then(res => {
        if(res.code === '0000')
