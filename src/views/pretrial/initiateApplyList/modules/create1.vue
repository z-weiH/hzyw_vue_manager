<template>
  <div>
    <el-dialog title="提示" :visible.sync="$parent.editState1" width="47%">
      <div v-if="zqdata.resultType === 1">
        ...
        <ul>
          <li>
            申请
            <span class="f_orange">{{zqdata.totalCase}}</span>件案件，共需消耗
            <template v-if="zqdata.settleType != 2">
              仲券
              <span class="f_orange">{{zqdata.needTicketCount}}</span>张，
            </template>
            <template v-else>
              服务费
              <span class="f_orange">{{zqdata.needTicketFeeCount}}</span>元，
            </template>
            受理费
            <span>{{zqdata.totalAmt}}</span>元。
          </li>
          <li>该平台账户仲券及受理费充足，可以申请立案。</li>
          <li>
            确定对
            <span class="f_orange">{{zqdata.totalCase}}</span>件案件申请立案？
          </li>
        </ul>
      </div>
      <div v-if="zqdata.resultType === 2">
        ...
        <ul>
          ...
          <li>
            该平台账户仲券或受理费不足，仅能对
            <span class="f_orange">{{zqdata.factCaseCount}}</span>件案件申请立案。
          </li>
          <li>
            确定对
            <span class="f_orange">{{zqdata.factCaseCount}}</span>件案件申请立案？
      ...
      <span slot="footer" class="dialog-footer">
        <customer-button
          type="primary"
          @click="confirmApCase"
          :disabled="zqdata.resultType === 3 || disabled"
        >确 定</customer-button>
...
export default {
  ...
  methods: {
    confirmApCase() {
      ...
      this.disabled = true;
      this.$http
        .post(URL_JSON["confirmApplyCase"], this.itsObj)
        .then(res => {
          console.log("成功！", res.result);
          this.$parent.$parent.initQuery(
            this.$parent.$parent.queryUrl,
            this.$parent.$parent.searchItem
          );
