    ...
    <div class="search">
        <el-row :gutter="10">
          <div v-for="(search, index) in searchItems" :key="index">
              <search-item :item="item" v-bind:value="item[search.property]" v-bind:search-item="search" @valueChange="valueChange"></search-item>
          </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      valueChange(key) {
        let obj = {property: key, value: this.item[key]};
