...
export default {
  ...
  methods: {
    ...
    handleMousewheel(e) {
      // 滚轮向上
      if(e.deltaY < 0) {
        this.$emit('update:scale',this.scaleMultiple[this.scaleMultiple.indexOf(this.scale) + 1] || this.scaleMultiple[this.scaleMultiple.length - 1])
      }else{
        this.$emit('update:scale',this.scaleMultiple[this.scaleMultiple.indexOf(this.scale) - 1] || this.scaleMultiple[0]);
      }
    },
  }
};
